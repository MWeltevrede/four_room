...
class NxNRoomsEnv(MiniGridEnv):

    """
    ### Description

    Classic NxN room reinforcement learning environment. The agent must
    navigate in a maze composed of NxN rooms interconnected by gaps in the
    walls. To obtain a reward, the agent must reach the green goal square. Both
    the agent and the goal square are randomly placed in any of the NxN rooms.
    ...
    """
    @staticmethod
    def valid_positions(N, room_size):
        """
        ...
        # loop through the NxN rooms:
        for i in range(0,N):
            for j in range(0,N):
                # every room has room_size**2 possible locations
                for k in range(0,room_size):
                    for l in range(0,room_size):
                        pos = (k + 1 + i + i*room_size, l + 1 + j + j*room_size)
                        valid_agent_pos.append(pos)
                        valid_goal_pos.append(pos)

        # number of inside edges in an (N+1)x(N+1) grid graph
        num_walls = 2*(N+1)**2 - 2*(N+1) - 4*N
        valid_doors_pos = list(product(list(range(0, room_size)), repeat=num_walls))
        
        return valid_agent_pos, valid_goal_pos, valid_doors_pos


    def __init__(self, agent_pos=None, agent_dir=None, goal_pos=None, doors_pos=None, max_steps=100, N=2, room_size=3, **kwargs):
        """
            Parameters
            ----------
            agent_pos : list
        ...
        super().__init__(
            ...
            width=N+1+(N*room_size),
            height=N+1+(N*room_size),
            max_steps=max_steps,
            highlight=False,
            **kwargs
        )
    ...
    def _gen_grid(self, width, height):
        ...
        if self._doors_pos_list is not None:
            ...
            num_walls = 2*(self.N+1)**2 - 2*(self.N+1) - 4*self.N
            doors_pos = [self._rand_int(0, self.room_size) for _ in num_walls]
            agent_dir = self._rand_int(0, 4)


        # Define internal walls
        for i in range(0, self.N-1):
            self.grid.horz_wall(0, 1 + i + i*self.room_size + self.room_size)
            self.grid.vert_wall(1 + i + i*self.room_size + self.room_size, 0)

        # Create vertical doorways
        for i in range(0, self.N-1):
            # column of rooms except the last one
            for j in range(0, self.N):
                # every row
                pos = (1 + i + i*self.room_size + self.room_size, 1 + j + j*self.room_size + doors_pos[i*self.N + j])
                self.grid.set(*pos, None)

        # Create horizontal doorways
        for i in range(0, self.N-1):
            # row of rooms except the last one
            for j in range(0, self.N):
                # every column
                pos = (1 + j + j*self.room_size + doors_pos[self.N*(self.N-1) + i*self.N + j], 1 + i + i*self.room_size + self.room_size)
                self.grid.set(*pos, None)
